...
export class OAuthClient {
    ...
    async getAppAccessToken() {
        ...
        try {
            ...
            throw new EbayAuthError(msg, err.status ?? 0);
        }
        this.appTokenCache = {
    ...
    async getUserAccessToken() {
        ...
        if (!this.refreshToken) {
            throw new EbayAuthError("User access token required but no refreshToken provided. Call generateAuthUrl() and exchange the code first.", 0);
        }
     ...
     */
    generateAuthUrl(redirectUri, scopes, state) {
        const env = this.opts.env ?? "production";
